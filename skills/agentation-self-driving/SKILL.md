...
# If "NOT FOUND": Agentation is not mounted on this page — stop and tell the user to run
# /agentation. In a non-React host (Rails, Django, Laravel), also confirm the dev-only
# bundle entry was actually built and the server is running in development — the toolbar
# is deliberately absent from production builds.
...
- **Toolbar not found on page**: Agentation isn't mounted — run `/agentation` to set it up first
- **Toolbar vanished after a link click**: In a non-React host the toolbar is a React portal into `document.body`, and server-rendered navigation (Turbo/Hotwire, htmx boosting) replaces the body underneath it. The mount script must re-mount on the framework's navigation event. Re-run `/agentation` if those listeners are missing; meanwhile, stay on one page and scroll rather than following links
