...
You can run the command below to register your servers using [Toolbelt](../toolbelt/using-cloud66-toolbelt.md):
...
You can add registered servers to Cloud 66 via an on-premises [gateway server](../deployment/deployment-gateway.md) as long as:
...
- For **Kubernetes clusters**, we strongly recommend that your servers meet [these minimum specs](../servers/non-recommended-server-sizes.md) to be able to handle the additional load required by the platform.
- **Operating system**: We currently support Ubuntu 26.04 LTS, Ubuntu 24.04 LTS, or Ubuntu 22.04 LTS. The OS needs to be *freshly installed* on your server.
...
- **Firewalls & security:** Cloud 66 needs the following (TCP) ports open to allow us to deploy and manage your application:
    - Port `22` set to allow access from our [authorized IP addresses](../security/using-c66-via-firewall.md#cloud-66-s-authorized-ip-addresses)
    - Port `6443` set to allow access from our [authorized IP addresses](../security/using-c66-via-firewall.md#cloud-66-s-authorized-ip-addresses)
...
For more detail please read our guide to [Using Cloud 66 through firewalls](../security/using-c66-via-firewall.md).
...
For [CSv1](../cloud-66-101/concepts-and-terminology.md#version-1-vs-version-2) applications, all servers must be allowed to communicate inside the security group on TCP port `6783`, which is needed to create the overlay network (Weave). 
