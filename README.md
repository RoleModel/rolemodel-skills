...
| **[standup-prep](skills/standup-prep)** | Interview-style prep for a daily standup call. Asks which project/customer to focus on and whether the person is leading it — a lead gets a second, separate cross-project pass across Linear, GitHub, and Slack on top of their own personal view — pulls context from those sources plus the local git repo and Almanac, flags inconsistencies between Linear cards and GitHub PRs, then interviews the person to produce read-aloud prose answering the four standup questions (plus a project-status section for leads) — including opening questions to ask the customer. |
