...
| Skill | Description |
|-------|-------------|
| **[agentation](skills/agentation)** | Add the Agentation visual feedback toolbar to a project so you can point at UI problems in the browser and hand them to an agent. Built for Rails and other server-rendered hosts (Django, Laravel, plain webpack/Vite): adds React as a development-only dependency, with a production boundary verified against the bundler’s real output directory. |
