...
| **[pr-walkthrough](skills/pr-walkthrough)** | Structured, file-by-file walkthrough of a branch or PR that builds the reader's mental model of the change instead of hunting for bugs. Frames the central idea, orders files so the diff tells a story, batches 1–3 files at a time under a length and code-quoting budget, and recaps every flag once at the end. Understanding-oriented: never approves or submits a review. |
...
- **PR Walkthrough + Trace + Explain** — PR Walkthrough builds your mental model of one specific change, batch by batch; Trace follows a single code path through the stack when a batch raises a question the diff cannot answer; Explain orients you to the feature area the change lands in.
