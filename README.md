...
| **[standup-prep](skills/standup-prep)** | Interview-style prep for a daily standup call. Asks which project/customer to focus on, pulls context from Linear, GitHub, the local git repo, Almanac, and Slack scoped to that engagement, flags inconsistencies between Linear cards and GitHub PRs, then interviews the person to produce read-aloud prose answering the four standup questions — including opening questions to ask the customer. |
