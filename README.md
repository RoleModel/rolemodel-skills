...
| **[cloud66-deploy-v2](skills/cloud66-deploy-v2)** | The full Cloud 66 Deploy v2 documentation (300 pages) mirrored locally as Markdown, with an `INDEX.md` and a `scripts/refresh.rb` that re-downloads it from the sitemap. Answer Cloud 66 questions — `manifest.yml`, `service.yml`, deploy hooks, the `cx` toolbelt, servers, databases, load balancers — from the docs instead of from memory or a network fetch. Covers v2 only, not v3. |
