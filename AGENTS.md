...
- **agentation**: Add the Agentation visual feedback toolbar to a project. Targets Rails and other server-rendered hosts (Django, Laravel, Phoenix, plain webpack/Vite): adds React as a **development-only** dependency and mounts through a separate dev-only bundle entry. A host that already bundles React renders the component behind a dev-only dynamic import instead. Three layered guards keep React out of production: dev dependency, bundler-mode-gated entry, and a server-env-gated script tag — the bundler-mode check is the one that actually matters, and the skill verifies it with a production build. Notes the Turbo/htmx re-mount listeners the React portal needs, and points at `agentation-mcp` (port 4747) for syncing annotations to an agent.
- **agentation-self-driving**: Autonomous design critique using the Agentation toolbar. Drives a **visible headed** browser via `agent-browser` — never headless, so the user can watch — and adds 5-8 specific, actionable annotations per page. Requires the toolbar to already be mounted (run `/agentation` first). Documents the coordinate-click sequence (`mouse move` → `down` → `up`) that the overlay intercepts, since `click @ref` passes straight through to the DOM, plus the `@ref`-vs-CSS-selector split across `agent-browser` commands. `references/two-session-workflow.md` covers the full loop where a second session blocks on `agentation_watch_annotations` and fixes each annotation in code.
...
- The two `agentation` skills are a pair: `agentation` installs and mounts the toolbar, `agentation-self-driving` drives it. Setup changes belong in the former, browser-automation details in the latter — keep the production-boundary guidance in one place rather than duplicating it.
