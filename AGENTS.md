...
- **walk-pr**: Produce a reviewer's guide to a pull request rather than a review of it. Emits, in order: the spine (the 2-5 files that carry the change, in reading order, with the rest marked mechanical); the decisions the diff makes that its ticket doesn't settle, each as a question anchored to a line; blast radius computed by grep on the PR head (call sites, and existing specs that cover changed code but weren't updated); what a user would see, including rendered documents; and what it could not determine. Three constraints: shorter than the diff, questions rather than verdicts, and never cite a file or value it has not read. Complements an automated code review, which owns defect claims.
