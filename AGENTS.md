...
- **pr-walkthrough**: Structured, file-by-file walkthrough of a branch or PR built to give the reader a mental model of the change, not a bug list. Frames the central idea before the first file, orders files so the diff tells a story, then works through them 1–3 at a time under a hard budget (300–450 words and one to two short code blocks per batch) with flags surfaced above the explanation. Every flag is recapped exactly once at the close; an optional closing pass re-fetches the branch, re-walks new commits, and reconciles each flag as fixed / commented / accepted / still open. Understanding-oriented — never approves, rejects, or submits a review. Pairs with `file-pr` and `split-stack` on the review side of the same workflow.
