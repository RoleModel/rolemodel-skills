...
- Both `agentation` skills are forks of upstream skills in [benjitaylor/agentation](https://github.com/benjitaylor/agentation), pinned in a note at the top of each SKILL.md. The toolbar internals they depend on (`[data-feedback-toolbar]`, `[class*=toggleContent]`, `[data-annotation-marker]`) are private, so re-read upstream when bumping the `agentation` package. Keep the upstream commit in those notes current.
