...
- **cloud66-deploy-v2**: All 300 pages of the Cloud 66 Deploy v2 documentation, mirrored as Markdown under `references/` with an `INDEX.md` mapping every title to its local path and canonical URL. `scripts/refresh.rb` re-downloads from the sitemap (each help.cloud66.com page is published as Markdown at the same URL with `.md` appended) and rewrites the upstream's `/:product/:version?/...` router-template links into working relative paths. Scoped to Deploy **v2** — v2 clusters are coupled to their servers, v3 is Kubernetes-native with node pools and RA/HA cluster types, so v3 pages may contradict this.
